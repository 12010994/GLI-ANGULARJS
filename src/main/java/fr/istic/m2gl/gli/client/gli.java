			...
			public void onClick(ClickEvent event) {
				...
				rb.setCallback(new RequestCallback() {
					...
					public void onResponseReceived(Request request, Response response) {
						Window.alert("RESPONSE STATUS CODE:" + response.getStatusCode() + " REPONSE SIZE: " + response.getText().length());

						EventItf event = EventJsonConverter.getInstance()
								.deserializeFromJson(response.getText());
						
						Window.alert("Event number:" + event.getId()+" - "
									+"Place: "+event.getDate().toString());
